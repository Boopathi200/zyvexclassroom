...
async function connectDb() {
  ...
  if (uri.includes('localhost') && process.env.NODE_ENV === 'production') {
    console.warn('WARNING: Using localhost MongoDB in production. This will fail on hosted platforms.');
  ...
  try {
    ...
    if (error.message.includes('authentication failed')) {
      ...
      console.error('Whitelist 0.0.0.0/0 or your specific hosting provider IP.');
    }
    throw error;
  }
}
